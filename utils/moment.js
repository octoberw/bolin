...
Moment.prototype.format = function (format) {
  ...
  if (/(E+)/.test(format))
    format = format.replace(RegExp.$1, ((RegExp.$1.length > 1) ? (RegExp.$1.length > 2 ? "/u661f/u671f" : "周") : "") + week[date.getDay() + ""]);
  for (var k in o) {
...
Moment.prototype.differ = function (firstDate, secondDate) {
  var firstDate = new Date(firstDate);
  var secondDate = new Date(secondDate);
  var diff = Math.abs(firstDate.getTime() - secondDate.getTime())
  var differ = parseInt(diff / (1000 * 60 * 60 * 24));
  return differ
}
